...
describe('zoomReducer', () => {
  it('narrows to the dragged span on release', () => {
    ...
    expect(s).toEqual({ range: { from: 2, to: 5 }, drag: null });
  });
  ...
  it('ignores a click — one point is not a selection', () => {
    ...
    expect(run([{ type: 'down', index: 3 }, { type: 'up' }])).toEqual(ZOOM_IDLE);
    expect(run([{ type: 'down', index: 3 }, { type: 'move', index: 3 }, { type: 'up' }])).toEqual(ZOOM_IDLE);
  });
  ...
  it('arms a drag from a touch start and anchors it on the first point reported', () => {
    ...
    // Lifting an armed-but-unplaced finger is a tap.
    expect(zoomReducer(armed, { type: 'up' })).toEqual(ZOOM_IDLE);
